...
static void dsi_bridge_pre_enable(struct drm_bridge *bridge)
{
	...
	switch (dfpsflag) {
	case 144:
	dsi_panel_set_fps(c_bridge->display->panel, DSI_CMD_SET_144_ON);
	DSI_INFO("144hz 2F 00\n");
		break;
	case 120:
	dsi_panel_set_fps(c_bridge->display->panel, DSI_CMD_SET_120_ON);
	DSI_INFO("120hz 2F 01\n");
		break;
	case 90:
	dsi_panel_set_fps(c_bridge->display->panel, DSI_CMD_SET_90_ON);
	DSI_INFO("90hz 2F 02\n");
		break;
	case 60:
	dsi_panel_set_fps(c_bridge->display->panel, DSI_CMD_SET_60_ON);
	DSI_INFO("60hz 2F 03\n");
		break;
	default:
		break;
