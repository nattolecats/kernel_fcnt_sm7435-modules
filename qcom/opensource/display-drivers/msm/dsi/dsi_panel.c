		...
		struct msm_io_res *io_res)
{
	...
	dp_gpio = utils->get_named_gpio(utils->data,
					      "qcom,platform-dp-gpio", 0);
	DSI_ERR("dp_gpio:%d\n", dp_gpio);
...
int dsi_panel_set_fps(struct dsi_panel *panel, int mode)
{
	int rc = 0;
	struct mipi_dsi_device *dsi;

	if (!panel) {
		DSI_ERR("dfps invalid params\n");
		return -EINVAL;
	}
	dsi = &panel->mipi_device;
	...
	if (!dsi_panel_initialized(panel)) {
		DSI_ERR("dfps invalid status when set hbm mode.\n");
		goto panel_init_err;
	}

	printk(KERN_ERR "drm [DDI] %s is %u\n", __func__, mode);

	rc = dsi_panel_tx_cmd_set(panel, mode);
	DSI_ERR("code tx done\n");
