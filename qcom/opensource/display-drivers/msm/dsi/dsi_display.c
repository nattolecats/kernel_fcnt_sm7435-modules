...
void dsi_display_pcd_check(struct drm_connector *connector,struct dsi_display *display,char *rx_buf)
{

	u8 *rx_tmp = NULL;
	int rx_len = 0;
	unsigned char buffer[10] = {0x01,0x06,0x01,0x00,0x01,0x00,0x00,0x01,0xD0};
	unsigned char cmd_buffer[13] = {0x39,0x01,0x00,0x00,0x00,0x00,0x06,0xF0,0x55,0xAA,0X52,0x08,0x01};
	...
	if(!PCD_CHECK_READ_CMD_LEN)
        {
		DSI_ERR("%s:PCD_CHECK_READ_CMD_LEN mem failed!");
          return;
        }
  	if(!display)
        {
          DSI_ERR("%s:display  mem failed!");
          return;
        }

	dsi_display_cmd_transfer(connector,display,cmd_buffer,13);
	rx_len = dsi_display_cmd_receive(display,buffer + 1,PCD_CHECK_READ_CMD_LEN - 1,rx_tmp,buffer[0]);

	if(strlen(rx_tmp) == 0)
	{
		DSI_ERR("%s rx_tmp send failed\n",__func__);
		return;
	}
	...
	DSI_INFO("%s:rx_buf=%x",__func__,rx_buf[0]);

	if(((rx_buf[0] & 0x7F) <= 0x4C) && ((rx_buf[0] & 0x7F) != 0))
	{
		DSI_INFO("%s:PCD check ok",__func__);

	}else{
		pcdtrigger = 1;
		DSI_ERR("%s:PCD check fail",__func__);
			 ...
			 u32 flags)
{
	...
	if(currefersh != timing.refresh_rate){

		if((currefersh == 60) && (timing.refresh_rate == 90)){

		dsi_panel_set_fps(display->panel, DSI_CMD_SET_90_ON);
		DSI_INFO("60to90 90hz 2F 02\n");

		dfps6090 = 0;

		}

		if((currefersh == 90) && (timing.refresh_rate == 60)){

		dsi_panel_set_fps(display->panel, DSI_CMD_SET_60_ON);
		DSI_INFO("90to60 60hz 2F 02\n");
		...
		switch(currefersh) {
			case 144:
			dfpsline = 144;
			currefersh = timing.refresh_rate;
			break;
			case 120:
			dfpsline = 120;
			currefersh = timing.refresh_rate;
