		...
		bool enable, int orientation)
{
	...
	if (enable) {
		if(gpio_is_valid(dp_aux_oe)) {
			gpio_direction_output(dp_aux_oe, 0);
		}
		switch (orientation) {
		case ORIENTATION_CC1:
			...
			if(gpio_is_valid(dp_aux_en_gpio)) {
				...
				DP_AUX_ERR(dp_aux, "FSA_USBC_ORIENTATION_CC1 dp_aux_en_gpio output low");
			}
			break;
		case ORIENTATION_CC2:
			...
			if(gpio_is_valid(dp_aux_en_gpio)) {
				...
				DP_AUX_ERR(dp_aux, "FSA_USBC_ORIENTATION_CC2 dp_aux_en_gpio output High");
			}
			break;
		default:
		...
		if(gpio_is_valid(dp_aux_oe)) {
			gpio_direction_output(dp_aux_oe, 1);
			DP_AUX_ERR(dp_aux, "!enable dp_aux_oe output High");
	}
}
