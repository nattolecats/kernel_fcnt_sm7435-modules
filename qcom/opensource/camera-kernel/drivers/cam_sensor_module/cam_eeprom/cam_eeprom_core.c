	...
	struct cam_eeprom_memory_block_t *block)
{
	...
	uint32_t                           eeprom_read_page_size;
	uint32_t                           eeprom_read_page_addr;
	uint32_t                           eeprom_read_page_end;
	uint32_t                           eeprom_read_page_offset;
	uint32_t                           eeprom_read_page_rlen;
	uint32_t                           eeprom_read_page_data;
	struct cam_sensor_i2c_reg_array    eeprom_s5k4h7_cached_reg_array[] = {
											{.reg_addr=0x3B41, .reg_data=1},
											{.reg_addr=0x3B42, .reg_data=3},
											{.reg_addr=0x3B40, .reg_data=1},
											{.reg_addr=0x0A00, .reg_data=1},
											{.reg_addr=0x0A00, .reg_data=4},
											{.reg_addr=0x0A00, .reg_data=0},
										};
	uint8_t                            eeprom_loop_counter = 0;
	uint8_t                            eeprom_s5k4h7_readcheck_max_time = 100;
	...
	for (j = 0; j < block->num_map; j++) {
		...
		if (emap[j].mem.valid_size) {
			if (eb_info->i2c_info.slave_addr == 0x5A) { // for s5k4h7 eeprom
				// calculate page
				CAM_ERR(CAM_EEPROM, "eeprom find i2c address successfully");
				eeprom_read_page_size = 64;
				eeprom_read_page_offset = 0x0A04;
				eeprom_read_page_rlen = eeprom_read_page_size;
				eeprom_read_page_addr = (emap[j].mem.addr / eeprom_read_page_size);
				eeprom_read_page_end = emap[j].mem.addr + emap[j].mem.valid_size;

				if ((eeprom_read_page_addr * eeprom_read_page_size) < emap[j].mem.addr) {
					eeprom_read_page_offset += emap[j].mem.addr - (eeprom_read_page_addr * eeprom_read_page_size);
					eeprom_read_page_rlen = eeprom_read_page_rlen + 0x0A04 - eeprom_read_page_offset;

					if (eeprom_read_page_rlen > emap[j].mem.valid_size) {
						eeprom_read_page_rlen = emap[j].mem.valid_size;
					}
				}

				while ((eeprom_read_page_addr * eeprom_read_page_size) < eeprom_read_page_end) {
					// switch page and loop read util success
					...
					i2c_reg_array.reg_data = eeprom_read_page_addr; // page start from 0
					i2c_reg_array.delay = 0;
					i2c_reg_settings.reg_setting = &i2c_reg_array;
					rc = camera_io_dev_write(&e_ctrl->io_master_info,
							&i2c_reg_settings);

					for (eeprom_loop_counter = 0; eeprom_loop_counter < 4; eeprom_loop_counter++) {
						i2c_reg_settings.reg_setting = &eeprom_s5k4h7_cached_reg_array[eeprom_loop_counter];
						rc = camera_io_dev_write(&e_ctrl->io_master_info,
							&i2c_reg_settings);
					}

					for (eeprom_loop_counter = 0;
						 eeprom_loop_counter < eeprom_s5k4h7_readcheck_max_time;
						 eeprom_loop_counter++) {
						rc = camera_io_dev_read(&e_ctrl->io_master_info,
							0x0A01, &eeprom_read_page_data,
							CAMERA_SENSOR_I2C_TYPE_WORD,
							CAMERA_SENSOR_I2C_TYPE_BYTE);
						if (eeprom_read_page_data == 1) {
							break;
						}
					...
					if (eeprom_read_page_data != 1) {
						CAM_ERR(CAM_EEPROM, "s5k4h7 failed to switch page %d", eeprom_read_page_addr);
						return -EBUSY;
					}

					// read data
					rc = camera_io_dev_read_seq(&e_ctrl->io_master_info,
							eeprom_read_page_offset,
							memptr,
							CAMERA_SENSOR_I2C_TYPE_WORD,
							CAMERA_SENSOR_I2C_TYPE_BYTE,
							eeprom_read_page_rlen);
					if (rc < 0) {
						CAM_ERR(CAM_EEPROM, "read failed rc %d",
							rc);
						return rc;
					}
					memptr += eeprom_read_page_rlen;

					// reset data
					eeprom_read_page_offset = 0x0A04;
					eeprom_read_page_rlen = eeprom_read_page_size;
					eeprom_read_page_addr++;
					if (((eeprom_read_page_addr+1) * eeprom_read_page_size) > eeprom_read_page_end) {
						eeprom_read_page_rlen = eeprom_read_page_rlen + eeprom_read_page_end - (eeprom_read_page_addr * eeprom_read_page_size);
					}
				}

				for (eeprom_loop_counter = 4; eeprom_loop_counter < 6; eeprom_loop_counter++) {
					i2c_reg_settings.reg_setting = &eeprom_s5k4h7_cached_reg_array[eeprom_loop_counter];
					rc = camera_io_dev_write(&e_ctrl->io_master_info,
						&i2c_reg_settings);
				}
