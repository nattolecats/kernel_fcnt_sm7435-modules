...
static int32_t is_wu_actuator_power_cam = 0;
static int32_t is_wu_actuator_power_oth = 0;
static char *wu_actuator = "ac15000.qcom,cci0:qcom,actuator2";
extern bool is_ois_fw_update_done(void);
	...
	struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (i2c_list->op_code == CAM_SENSOR_I2C_WRITE_RANDOM) {
		if (strcmp(wu_actuator, a_ctrl->soc_info.dev_name) == 0) {
			if (is_wu_actuator_power_oth == 1 && is_wu_actuator_power_cam == 1)
			{
				CAM_INFO(CAM_ACTUATOR, "skip set vcm code");
				return rc;
			}
		}
		if(!is_ois_fw_update_done()){
			CAM_INFO(CAM_ACTUATOR, "OIS skip  vcm code");
			return rc;
		}
	...
	void *arg)
{
	...
	case CAM_ACTUATOR_PACKET_OPCODE_INIT:
		...
		if (a_ctrl->cam_act_state == CAM_ACTUATOR_ACQUIRE) {
			CAM_INFO(CAM_ACTUATOR, "power up other value %d, power up cam value %d.",
				is_wu_actuator_power_oth, is_wu_actuator_power_cam);

			if (strcmp(wu_actuator, a_ctrl->soc_info.dev_name) == 0) {
				CAM_INFO(CAM_ACTUATOR, "cam_actuator_power_up is uw actuator");
				if (is_wu_actuator_power_oth == 0) {
	...
	void *arg)
{
	...
	case CAM_RELEASE_DEV: {
		...
		if (a_ctrl->cam_act_state == CAM_ACTUATOR_CONFIG) {
			CAM_INFO(CAM_ACTUATOR, "power down other value %d, power down cam value %d.",
				is_wu_actuator_power_oth, is_wu_actuator_power_cam);

			if (strcmp(wu_actuator, a_ctrl->soc_info.dev_name) == 0) {
				CAM_INFO(CAM_ACTUATOR, "cam_actuator_power_down is uw actuator");
				if (is_wu_actuator_power_oth == 0) {
					rc = cam_actuator_power_down(a_ctrl);
					if (rc < 0) {
						CAM_ERR(CAM_ACTUATOR,
...
int32_t cam_actuator_power_on_from_other(struct cam_actuator_ctrl_t *a_ctrl)
{
	int32_t ret = 0;

	CAM_INFO(CAM_ACTUATOR, "power on from other value %d, power from cam value %d.",
			is_wu_actuator_power_oth, is_wu_actuator_power_cam);
	mutex_lock(&(a_ctrl->actuator_mutex));
	if (is_wu_actuator_power_cam == 0 && is_wu_actuator_power_oth == 0)
	{
		ret = cam_actuator_power_up(a_ctrl);
		if (ret < 0) {
			CAM_ERR(CAM_ACTUATOR," Actuator Power up failed");
			goto release_mutex;
		}
	}
	is_wu_actuator_power_oth = 1;
...
int32_t cam_actuator_power_off_from_other(struct cam_actuator_ctrl_t *a_ctrl)
{
	int32_t ret = 0;

	CAM_INFO(CAM_ACTUATOR, "power off from other value %d, power from cam value %d.",
		is_wu_actuator_power_oth, is_wu_actuator_power_cam);
	mutex_lock(&(a_ctrl->actuator_mutex));
	if (is_wu_actuator_power_cam == 0 && is_wu_actuator_power_oth == 1)
	{
		ret = cam_actuator_power_down(a_ctrl);
		if (ret < 0) {
			CAM_ERR(CAM_ACTUATOR," Actuator Power down failed");
		}
	}
	is_wu_actuator_power_oth = 0;
