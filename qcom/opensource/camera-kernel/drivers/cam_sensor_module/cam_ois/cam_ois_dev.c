...
extern int aw86006_ois_init(struct cam_ois_ctrl_t *o_ctrl); /* zpf awinic add */
extern int aw86006_ois_exit(struct cam_ois_ctrl_t *o_ctrl); /* zpf awinic add */
	...
	struct device *master_dev, void *data)
{
	...
	o_ctrl->cam_ois_state = CAM_OIS_INIT;
	aw86006_ois_init(o_ctrl); /* zpf awinic add */
	...
	struct device *master_dev, void *data)
{
	...
	v4l2_set_subdevdata(&o_ctrl->v4l2_dev_str.sd, NULL);
	/* aw86006_ois_exit(o_ctrl); */ /* zpf awinic add */
	kfree(o_ctrl);
}
